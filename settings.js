...
export const DEFAULT_SETTINGS = Object.freeze({ destination: Object.freeze({ mode: "default", subfolder: "" }), duplicateHandling: "uniquify", concurrentDownloadLimit: 3 });
...
const PORTABLE_BACKUP_KEYS = ["duplicateHandling", "concurrentDownloadLimit", "selectedFileTypes", "customCategories", "customCategorySelections"];
...
async function savePortable(values) { chrome.storage.sync.set(values).catch(() => {}); }
export async function loadSettings() { const local = await chrome.storage.local.get(["destination", ...PORTABLE_BACKUP_KEYS]); const synced = await chrome.storage.sync.get(PORTABLE_BACKUP_KEYS); const saved = { ...synced, ...local }; if (!Object.keys(local).some((key) => PORTABLE_BACKUP_KEYS.includes(key)) && Object.keys(synced).length) chrome.storage.local.set(synced).catch(() => {}); const customCategories = normalizeCustomCategories(saved.customCategories); return { settings: normalizeSettings(saved), selectedFileTypes: normalizeSelectedFileTypes(saved.selectedFileTypes), customCategories, customCategorySelections: normalizeCustomSelections(saved.customCategorySelections, customCategories) }; }
export async function saveSettings(settings) { await chrome.storage.local.set({ destination: settings.destination, duplicateHandling: settings.duplicateHandling, concurrentDownloadLimit: settings.concurrentDownloadLimit }); savePortable({ duplicateHandling: settings.duplicateHandling, concurrentDownloadLimit: settings.concurrentDownloadLimit }); }
export async function saveSelectedFileTypes(selectedFileTypes) { await chrome.storage.local.set({ selectedFileTypes }); savePortable({ selectedFileTypes }); }
export async function saveCustomCategories(customCategories, customCategorySelections) { await chrome.storage.local.set({ customCategories, customCategorySelections }); savePortable({ customCategories, customCategorySelections }); }
export async function loadDownloadedFileCount() { const local = await chrome.storage.local.get("downloadedFileCount"); const saved = Object.hasOwn(local, "downloadedFileCount") ? local : await chrome.storage.sync.get("downloadedFileCount"); const count = Number.isSafeInteger(saved.downloadedFileCount) && saved.downloadedFileCount >= 0 ? saved.downloadedFileCount : 0; if (!Object.hasOwn(local, "downloadedFileCount") && Object.hasOwn(saved, "downloadedFileCount")) chrome.storage.local.set({ downloadedFileCount: count }).catch(() => {}); return count; }
...
export function concurrentDownloadLimitLabel(value) { return { 1: "Sequential", 3: "3", 5: "Faster (5)", all: "Start all at once" }[normalizeConcurrentDownloadLimit(value)]; }
