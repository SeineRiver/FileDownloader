...
export async function loadSettings() { const saved = await chrome.storage.local.get(["destination", "duplicateHandling", "concurrentDownloadLimit", "selectedFileTypes", "customCategories", "customCategorySelections"]); const customCategories = normalizeCustomCategories(saved.customCategories); return { settings: normalizeSettings(saved), selectedFileTypes: normalizeSelectedFileTypes(saved.selectedFileTypes), customCategories, customCategorySelections: normalizeCustomSelections(saved.customCategorySelections, customCategories) }; }
export function saveSettings(settings) { return chrome.storage.local.set({ destination: settings.destination, duplicateHandling: settings.duplicateHandling, concurrentDownloadLimit: settings.concurrentDownloadLimit }); }
